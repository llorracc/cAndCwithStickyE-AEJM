...
\newcommand{ }{\ifthenelse{\boolean{numSwitch}}{}{\nonumber}}
...
\newcommand{\BoEYN}{\ifthenelse{\boolean{BoE}}}

\newcommand{\BoEHideBegin}{\ifthenelse{\boolean{BoE}}{\setboolean{includeTF}{false}}{}}
\newcommand{\BoEHideEnd}{  \ifthenelse{\boolean{BoE}}{\setboolean{includeTF}{true}}{}}

\newcommand{\TimeShortHideBegin}{\ifthenelse{\boolean{TimeShort}}{\setboolean{includeTF}{false}}{}}
\newcommand{\TimeShortHideEnd}{  \ifthenelse{\boolean{TimeShort}}{\setboolean{includeTF}{true}}{}}


\provideboolean{TimeShort}
\setboolean{TimeShort}{true}
%\setboolean{TimeShort}{false}
\newcommand{\TimeShortYN}{\ifthenelse{\boolean{TimeShort}}}
...
\newcommand{\TablesDir}{\TabsDir}
%\newcommand{\ParmDir}{C:/Jirka/Research/stickyConsumptionUS/Nov26_07/cAndCwithStickyEArchive/Calibration/Parameters}
%\newcommand{\TablesDir}{C:/Jirka/Research/stickyConsumptionUS/Nov26_07/cAndCwithStickyEArchive/Tables}
...
\definecolor{jirkasred}{rgb}{0.9,0,0}
\definecolor{jirkasblue}{rgb}{0.2,0.2,0.7}
\newcommand{\jemph}[1]{{\color{jirkasblue}#1}}
...
\begin{eqnarray*}
  ...
  \\      \cLevBF_{t+1} & = & (\rfree/\Rfree)\perc{\wAllLev}_{t+1} = (\rfree/\Rfree)     \wAllLev _{t} = \cLevBF_{t}
%  \\      \cLevBF_{t+2} & = & (\rfree/\Rfree)\perc{\wAllLev}_{t+2} = (\rfree/\Rfree)     \wAllLev _{t} = \cLevBF_{t}
  \\               \vdots  &  & \vdots \nonumber
...
\begin{eqnarray*}
   \mathbf{c}_{t+n}-\mathbf{c}_{t} & = & (\rfree/\Rfree)\underbrace{(\wAllLev_{t+n}-\wAllLev_{t}) }_{\Delta^{n} \wAllLev_{t+n}}
\end{eqnarray*}
...
{p}_{t+1,i} =
  \begin{cases}
    1 & \text{for newborns}
\\    {p}_{t,i} \psi_{t+1,i} & \text{for survivors}
  \end{cases}
\end{equation*}
...
\begin{eqnarray*}
       \KLevBF_{t+1} & = & \int_{0}^{1} \left(\frac{1-\pDies_{t+1,i}}{1-\PDies}\right) \aLevBF_{t,i}   \,\text{d}i   \label{eq:Ktp1} = \ALevBF_{t}
   \\          K _{t+1} & = &  \ALev_{t}\big/(\Psi_{t+1} \PtyGro_{t+1})  
\end{eqnarray*}
...
\begin{eqnarray*}
  \mathbf{a}_{t,i} & = & \mathbf{m}_{t,i}-\mathbf{c}_{t,i}
\end{eqnarray*}

\item Capital transition depends on prob of survival $1-\PDies$:
\begin{eqnarray*}
   \mathbf{k}_{t+1,i} & = &   \mathbf{a}_{t,i}\big/(1-\PDies)
\end{eqnarray*}
...
\begin{eqnarray}
 \Delta \log \mathbf{C}_{t+1} & \approx & \varsigma+\chi \Delta \log \mathbf{C}_{t}+\eta \Ex_{t}[\Delta \log \mathbf{Y}_{t+1}]+\alpha A_{t}+\epsilon_{t+1} \nonumber
\end{eqnarray}
...
\small
\begin{eqnarray}
 \CGrowCross    \label{eq:CGrowCross}     \nonumber
\end{eqnarray}
...
%\newcommand{\DirCampManVsStickyE}{/Volumes/Data/Work/cAndCwithStickyEArchive/Empirical/US/Results/LaTeX/tables}
...
\begin{eqnarray*}
      \hat{{P}}_{t+1} & = & \Pi \mathbf{Y}_{t+1} + (1-\Pi) \hat{{P}}_{t},
\end{eqnarray*}
